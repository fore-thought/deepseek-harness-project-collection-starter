...
> **生成源锚点**（用来判断该不该重新生成）：以下三份组合由 DSH `0.1.6-alpha.1` 的
> `standard/agent.cordis.yml` 生成，该源文件 SHA256：
> `942480B0E441277063F358A511F2727DC2EF8EE6A5E5E127CEE1FAEA3DAB0149`。
> DSH 升级后先算一遍源文件哈希：与上面不一致就说明标准模式变了，按下面三步重新生成。
> 版本号只是辅助线索，哈希才是判据。
>
> 另：`agent.cordis.yml` 的正文是 DSH 源文件的**逐字副本**，含它自带的长注释行（有几行超过
