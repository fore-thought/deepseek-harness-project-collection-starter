# 合规与工程规范（standards·项目组级）

> 项目组级全局规范：记录合规要求、工程规范与项目组特色约定，被各项目只读引用。
> 更新走提权通道（见 AGENTS.md §8/§9）。发布/商用前逐项核实。
> 版本: v1.5 ｜ 最近修订: 2026-09-16 ｜ 变更记录见文末。
> **横向定位**：本文件覆盖所有项目类型（内容生成/资源整理/其它），只写通用条款；
> 项目类型特有条目由各项目在项目级 AGENTS §2 或项目文档中自行维护，不写入本文件。

## 1. 内容合规

> 类别 + 核实机制：具体条目由各项目按自身类型填写，发布/商用前逐项核实。
> 核实凭证 = 来源 URL + 核实日期（记入本文件状态列或项目级合规清单）。
> 本节只定义**机制**，不预置具体类别——类别随项目类型走：内容生成类看生成物标识、
> 版权与素材许可；代码类看依赖许可证与开源义务；数据类看数据来源与个人信息；
> 资源整理类看原始许可与再分发条款。

- [ ] `<合规类别一>（来源：官方政策/平台规则；核实凭证：URL + 日期）`
- [ ] `<合规类别二>`
- [ ] `<各项目按类型补充的其他合规类别>`

## 2. 工程规范

- 文档格式：UTF-8（无 BOM）、Markdown、LF 换行（见 AGENTS.md §4）
- 文件名：小写字母 + 连字符，不用空格和中文文件名
- 结论分层：数字/阈值/事实性结论必标 [MEASURED]/[ESTIMATED]/[UNKNOWN]
  （见 AGENTS.md §2.4）
- 禁止本地绝对路径：文档/脚本中不出现用户本地绝对路径（如 `C:\...`），
  一律项目相对路径或中性描述（见 AGENTS.md §4）
- **敏感信息管理**：
  - 真实凭证（API Key/令牌/密码）放项目 `process/project/configs/.env`
    （gitignore 掉），同时保留 `configs/.env.template` 占位符版（可进 git）；
  - gitignore 至少排除：`*.env`、`secrets/`、`credentials/`、`*.pem`、`*token*`、
    `configs/project-config.toml`（项目配置，含远端 URL 等**非密钥**项，不入库；密钥见 `.env`）；
  - 组级 `user-config.toml` 放 `shared-standards/`（.gitignore 排除，不入库）；
  - 模板命名统一为「实例名 + `.template`」（`project-config.toml.template` /
    `user-config.toml.template` / `.env.template`），占位符版可入库
    （实例 = 去掉 `.template` 后缀，本地填写）；
  - 权限分级：运行所需令牌代理可见；支付/主账号类仅用户可见，代理需要时经用户注入；
  - 泄露应急：发现泄露 → 立即撤销/轮换 → 记 decisions → 复盘原因。
- **政策监控清单**：平台规则/口径变动统一登记在一份清单（每平台一行：
  公告渠道 + 最近核实日期 + 最近变动 + 影响面[指标字典?/合规?]）；
  指标字典与合规核实**共用此清单**，不各自维护两套。清单位置各项目自定
  （建议 `process/docs/` 或目标书附录）；渠道来源 = 平台官方公告。

## 3. 项目组特色约定

- `<项目组特色定制：行业惯例、发布渠道规则、品牌要求等，遇新规则补充>`
  （注意横向定位：只写对所有项目类型通用的约定）

## 变更记录

| 日期 | 版本 | 变更 | 批准 |
|---|---|---|---|
| 2026-08-27 | v1.0 | 首次版本化：横向化重构（类别+核实机制）/敏感信息管理/政策监控清单/结论分层强制范围 | 用户 |
| 2026-08-27 | v1.1 | git 远端预留文件 configs/git-remote.md 纳入不入库清单（会话中不留痕） | 用户 |
| 2026-08-27 | v1.2 | 配置预留文件升级 TOML 两级（project-config.toml 不入库 / user-config.toml 集合根 / *.template.toml 可入库） | 用户 |
| 2026-08-27 | v1.3 | user-config.toml 移至 shared-standards/（.gitignore 排除）；user-config.template.toml 入库 | 用户 |
| 2026-08-27 | v1.4 | 工程规范新增「禁止本地绝对路径」（见 AGENTS §4） | 用户 |
| 2026-09-16 | v1.5 | 通用性清理与命名回归：§1 内容合规改为只定义机制、不预置具体类别（原条目属内容型项目专用）；§2 模板命名统一为「实例名 + `.template`」（`.env.template` / `project-config.toml.template` / `user-config.toml.template`，原 `env.template` 与 `*.template.toml` 系把后缀插在扩展名之前，与「去掉 `.template` 即实例」的既定规则不符）。防的事故（频率×损失）：模板预置单一领域的合规类别与产物类型导致跨类型项目开局就带错假设（每建一个新类型项目必发，损失为开局返工）；模板名与实例名的推导规则不自洽导致代理复制错方向（每次首次配置，损失为配置失败） | 用户 |
